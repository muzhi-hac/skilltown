...
答得不到位不会当场公布答案，对方会**升级施压**。Sam、Nina、Jo 走固定阶梯（当成惯例 →
加码 → 打人情 → 要你别留记录），一轮一级；**Alex 按回答选策略**——只说拒绝就追问理由，
理由够了就追问怎么处理，一律拒绝就换成允许接受的情境试边界，答够了第一轮就让步。顶不住之后不是甩一句"你错了"，而是**"后来发生了什么"三拍展开**——一拍一点，
...
不会逼你把同一句话重复四遍。作答**全部由学习者自己打字**，没有任何选择题。构建产物 240KB JS（gzip 76KB）+ 10KB CSS，页面秒开；旧的 Godot 客户端保留在
`godot/` 但已不构建、不部署。

已验证的部分：

- `server/tests` 144 项通过；FastAPI 提供 `/ready` RAG 探针和 13 个接口操作，SQLite 保存匿名会话。
...
- Alex 的自适应由 `SKILLTOWN_ALEX_ADAPTIVE_ENABLED` 控制，默认关闭；关闭时他和其他人
  一样走固定阶梯。开启后策略仍由服务端从已校验的评分点信号（covered / missing /
