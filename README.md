...
答得不到位不会当场公布答案，对方会追问，**最多四轮**，顶不住才进入后果预演，再由唯一
不施压的角色 Mira 复盘。Sam、Nina、Jo 走固定阶梯（当成惯例 → 加码 → 打人情 → 要你别
留记录），一轮一级，与你答了什么无关；**Alex 按回答选策略**——只说拒绝就追问理由，理由
够了就追问怎么处理，一律拒绝就换成允许接受的情境试边界，答够了第一轮就让步。作答**全部
由学习者自己打字**，没有任何选择题。构建产物 240KB JS（gzip 76KB）+ 10KB CSS，页面秒开；旧的 Godot 客户端保留在
`godot/` 但已不构建、不部署。
