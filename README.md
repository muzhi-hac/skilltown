...
答得不到位不会当场公布答案，对方会**根据回答缺口继续施压**。Alex、Sam、Nina、Jo
都由服务端从已验证的评分点选择策略：只给决定就追问理由，理由齐了就追问执行，过度概括
就测试条件边界，明确承诺违规行动则提前进入后果。顶不住之后不是甩一句"你错了"，而是
**"后来发生了什么"三拍展开**——一拍一点，
时间往前走，波及面从你自己扩到同事、公司，最后又落回你的名字上（第 11 天审计调取礼品
登记 → 三月许可决定被重新打开、签字的处长停职 → 夏天你成了一份你从没打开过的卷宗里的
当事人）。看完才由唯一不施压的角色 Mira 复盘，并可倒回决策点重来。**一律拒绝的人看到的是另一条
链**——同一个后果节点接住两种相反的错误，所以故事按结局取，不会把没发生的事讲给你听。**说得含糊就一路走完四
级阶梯；把话说死了（"我就收下，这是我的答案"）只再推一轮**——而且直接上最狠的一级，
不会逼你把同一句话重复四遍。每位访客的模块、范围和训练技能可以在进门前或对话中查看；
每次正确决定先显示成功卡，由学习者选择继续当前模块或切换下一位访客。作答**全部由学习者
自己打字**，没有任何选择题。旧的 Godot 客户端保留在
`godot/` 但已不构建、不部署。

已验证的部分：

- `server/tests` 通过；FastAPI 提供 `/ready` RAG 探针和 13 个接口操作，SQLite 保存匿名会话。
...
- 施压台词由模型现场生成，但受服务端约束：引用了条款号或给出答案的台词会被丢弃，改说
  内容里写好的台词。判定和台词是同一次模型调用的两个字段，所以一轮仍然只花一次调用。
  人设与升级阶梯只存在于服务端，`/api/v1/town` 不下发（有测试盯着这条）。
- 全角色自适应由 `SKILLTOWN_ADAPTIVE_NPCS_ENABLED` 控制，默认关闭；旧的
  `SKILLTOWN_ALEX_ADAPTIVE_ENABLED` 作为兼容回退。关闭时所有角色走固定阶梯。开启后策略仍由
  服务端从已校验的评分点信号（covered / missing /
  overgeneralized / passed）重算，模型只是提议：提议不一致就只换掉那句台词，判定照用。
  学习者明确承诺做出该情境禁止的行动时会提前进入后果，这一步要求模型给出学习者本轮的
  连续原文并由服务端逐字核对。结构校验挡的是越权和编造，**不保证模型的语义判断永不出错**。
...
.venv/bin/python -m pytest server/tests -q                     # 144 passed
