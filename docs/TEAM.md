...
- Isso-W 已接受仓库协作邀请并拥有 write 权限；A/B 独立任务和共同任务均已绑定对应 GitHub Assignee。

## 文件归属

A：godot/、Dockerfile、根构建/部署配置、README 中的 Godot 导出和发布说明。
B：server/、docs/openapi.yaml、Pydantic API 模型、内容和评分配置。
API 契约由 B 提交、A 审核。改变字段、枚举、错误码前通知对方；Godot 与 FastAPI 不各自维护独立 ID 列表。
...
A：验证 Godot Web Export、移植地图、分类、点击/WASD/E、对话框、后果预演、页面发布。
B：FastAPI 会话、场景引擎、主线分支、持久记录、一次真实 AI 自由回答。
...
| [#2](https://github.com/muzhi-hac/skilltown/issues/2) | @muzhi-hac | [A][D1] Godot Web 小镇、分类与点击 / WASD 交互 |
| [#3](https://github.com/muzhi-hac/skilltown/issues/3) | @Isso-W | [B][D1] FastAPI 游客会话、场景 API 与持久任务状态 |
| [#4](https://github.com/muzhi-hac/skilltown/issues/4) | @muzhi-hac | [A][D1] Godot 培训对话、选项与可倒带后果预演 |
...
| [#7](https://github.com/muzhi-hac/skilltown/issues/7) | @muzhi-hac | [A][D2] Godot 学习护照、证据与可点击个人方案 |
